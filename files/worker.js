  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const routes = {
        ...
        'GET /api/channels': () => getChannels(env, url.searchParams),
        'GET /api/objectives': () => getObjectives(env, url.searchParams),
...
async function getChannels(env, params) {
  const db = env.DB;
  let startDate = params.get('startDate');
  let endDate = params.get('endDate');
  let whereClause = "";
...
async function getObjectives(env, params) {
  const db = env.DB;
  let startDate = params.get('startDate');
  let endDate = params.get('endDate');
  let whereClause = "";
...
async function getOrCreateCampaign(db, name) {
  ...
  if (name.toLowerCase().includes('ig') || name.toLowerCase().includes('instagram')) {
    platform = 'Instagram';
  }
  ...
  const result = await db.prepare(
    'INSERT INTO dim_campaign (campaign_name, objective, platform) VALUES (?, ?, ?) RETURNING campaign_id'
  ).bind(name, objective, platform).first();
